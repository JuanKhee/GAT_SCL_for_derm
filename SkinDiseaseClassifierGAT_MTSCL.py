...
class SkinDiseaseClassifier():
        ...
        for epoch in range(self.epochs):
            ...
            for batch in epoch_tracker:
                epoch_tracker.set_description(
                    f"loss: {np.average(epoch_loss) if len(epoch_loss) > 0 else None}; acc: {np.average(epoch_acc) if len(epoch_acc) > 0 else None}; last prediction: {outputs.max(2).indices.detach().cpu().numpy()[:,0] if outputs is not None else None}"
                )
                ...
                ce_outputs = self.model(h, adj, src, tgt, Msrc, Mtgt, Mgraph)
                # ce_outputs = torch.nn.Sigmoid()(ce_outputs)
                f1, f2 = torch.split(torch.nn.Sigmoid()(ce_outputs), [bsz, bsz], dim=0)
    ...
    def evaluate_model(self):
        ...
        for i, batch in enumerate(self.test_loader, 0):
            ...
            outputs = outputs.max(2).indices.detach().cpu().numpy()[:,0]
...
if __name__ == "__main__":
    ...
    dev_classifier = SkinDiseaseClassifier(
        ...
        epochs=2,
        batch_size=8,
        output_dir='dev_model_result_gat_mtscl',
