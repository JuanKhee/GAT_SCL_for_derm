...
class GATLayerEdgeSoftmax(nn.Module):
    ...
    def __init__(self, d_i, d_o, act=F.leaky_relu, eps=1e-6):
        ...
        # self._init_weights()
        with torch.no_grad():
            self.W_in.weight = torch.nn.Parameter(torch.ones((2 * d_i, d_o)).T)
            self.a.weight = torch.nn.Parameter(torch.ones((d_o, d_o)).T)
            self.W_out.weight = torch.nn.Parameter(torch.ones((d_i, d_o)).T)
...
class GAT_image(nn.Module):
    ...
    def __init__(self, num_features, num_classes, num_heads=[2, 2, 2], layer_sizes=[32,64,64]):
        ...
        print('self.GAT_layer_sizes[:-1]', self.GAT_layer_sizes[:-1])
        print('self.GAT_layer_sizes[1:]', self.GAT_layer_sizes[1:])
        print('self.layer_heads[:-1]', self.layer_heads[:-1])
        print('self.layer_heads[1:]', self.layer_heads[1:])
